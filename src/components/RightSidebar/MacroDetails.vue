<template>
  <div v-if="variableInfo" class="space-y-4">
    <div>
      <label class="block text-sm font-medium text-gray-700">变量</label>
      <div
    ...
    <div>
      <label class="block text-sm font-medium text-gray-700">
        定义位置 ({{ variableInfo.definedIn.length }})
      </label>
      <div class="mt-1 w-full">
        ...
        <div v-else class="rounded-md border border-red-200 bg-red-50 p-2">
          <p class="text-sm font-medium text-red-700">此变量在任何地方都未定义。</p>
        </div>
      </div>
    </div>
    ...
    <div>
      <label class="block text-sm font-medium text-gray-700">
        引用位置 ({{ variableInfo.referencedIn.length }})
      </label>
      <div class="mt-1 w-full">
        ...
        <p v-else class="mt-1 text-sm text-gray-500 italic">没有被任何其他提示词引用。</p>
      </div>
    </div>
  </div>
</template>
...
const getPromptName = (promptId) => {
  ...
  return prompt ? prompt.name : '未知提示词';
};
